...
export function createModuleNotFoundChain(
    ...
    const node10Result = host.getResolvedModule(
        sourceFile,
        moduleReference,
        mode,
    )?.node10Result;
    const result = node10Result
        ? chainDiagnosticMessages(
            ...
            createDiagnosticArgument(node10Result),
            createDiagnosticArgument(
                node10Result.includes(nodeModulesPathPart + "@types/")
        ...
        ? chainDiagnosticMessages(
            ...
            createDiagnosticArgument(packageName),
            createDiagnosticArgument(mangleScopedPackageName(packageName)),
        )
        ...
        ? chainDiagnosticMessages(
            ...
            createDiagnosticArgument(packageName),
            createDiagnosticArgument(moduleReference),
        )
        : chainDiagnosticMessages(
            /*details*/ undefined,
...
export function getNonDecoratorTokenPosOfNode(
    ...
    const lastDecorator = !nodeIsMissing(node) && canHaveModifiers(node)
...
/** @internal */
export const getScriptTargetFeatures = /* @__PURE__ */ memoize(
    (): ScriptTargetFeatures =>
        new Map(
            Object.entries({
                Array: new Map(
                    Object.entries({
...
export function getNonAugmentationDeclaration(symbol: Symbol) {
    return symbol.declarations?.find(
        d => !isExternalModuleAugmentation(d) &&
...
// Return display name of an identifier as a diagnostic argument
// Computed property names will just be emitted as "[<expr>]", where <expr> is the source
// text of the expression in the computed property.
/** @internal */
export function declarationNameToDiagnosticArgument(
    name: DeclarationName | QualifiedName | undefined,
): StructuredDiagnosticArgument {
    if (name === undefined) {
        return createDiagnosticArgument(declarationNameToString(name));
    }
    return createDiagnosticArgument(name, declarationNameToString, "Node");
}
 ...
 * @internal
 */
export function isVarAwaitUsing(
 ...
 * @internal
 */
export function isVarUsing(
    node: VariableDeclaration | VariableDeclarationList,
): boolean {
    return (
        (getCombinedNodeFlags(node) & NodeFlags.BlockScoped) === NodeFlags.Using
    );
}
...
export function isVarConst(
    ...
    return (
        (getCombinedNodeFlags(node) & NodeFlags.BlockScoped) === NodeFlags.Const
    );
}
...
export function isLet(node: Node): boolean {
    return (
        (getCombinedNodeFlags(node) & NodeFlags.BlockScoped) === NodeFlags.Let
    );
}
...
/** @internal */
export function getContainingClassExcludingClassDecorators(
    node: Node,
): ClassLikeDeclaration | undefined {
    const decorator = findAncestor(node.parent, n => isClassLike(n) ? "quit" : isDecorator(n));
    return decorator && isClassLike(decorator.parent)
        ? getContainingClass(decorator.parent)
...
export function classOrConstructorParameterIsDecorated(
    ...
    return (
        !!constructor && childIsDecorated(useLegacyDecorators, constructor, node)
    );
}
...
export function isExpressionNode(node: Node): boolean {
    switch (node.kind) {
        ...
        case SyntaxKind.ExpressionWithTypeArguments:
            return (
                !isHeritageClause(node.parent) && !isJSDocAugmentsTag(node.parent)
            );
        case SyntaxKind.QualifiedName:
        ...
        case SyntaxKind.ForOfStatement:
            ...
            return (
                (forInOrOfStatement.initializer === node &&
                    forInOrOfStatement.initializer.kind !==
                        SyntaxKind.VariableDeclarationList) ||
...
function isCompoundLikeAssignment(
    assignment: AssignmentExpression<EqualsToken>,
): boolean {
...
export function isInCompoundLikeAssignment(node: Node): boolean {
    ...
    return (
        !!target &&
        isAssignmentExpression(target, /*excludeCompoundAssignment*/ true) &&
...
export function getPropertyNameForPropertyNameNode(
    name: PropertyName | JsxAttributeName,
): __String | undefined {
    switch (name.kind) {
        case SyntaxKind.Identifier:
        case SyntaxKind.PrivateIdentifier:
...
export function isAnonymousFunctionDefinition(
    ...
    switch (node.kind) {
        case SyntaxKind.ClassExpression:
            if (
                classHasDeclaredOrExplicitlyAssignedName(node as ClassExpression)
            ) {
                return false;
            }
            break;
...
function getModifierFlagsWorker(
    ...
    if (alwaysIncludeJSDoc || (includeJSDoc && isInJSFile(node))) {
        if (
            ...
            node.modifierFlagsCache |= getRawJSDocModifierFlagsNoCache(node) |
...
function selectEffectiveModifierFlags(flags: ModifierFlags) {
    return (
        (flags & ModifierFlags.NonCacheOnlyModifiers) |
        ((flags & ModifierFlags.JSDocCacheOnlyModifiers) >>> 23)
...
export function tryGetPropertyAccessOrIdentifierToString(
    expr: Expression | JsxTagNameExpression,
): string | undefined {
    if (isPropertyAccessExpression(expr)) {
...
export function isRightSideOfQualifiedNameOrPropertyAccess(node: Node) {
    return (
        ...
        (node.parent.kind === SyntaxKind.PropertyAccessExpression &&
            (node.parent as PropertyAccessExpression).name === node) ||
        (node.parent.kind === SyntaxKind.MetaProperty &&
            (node.parent as MetaProperty).name === node)
    );
}
...
/** @internal */
export function isInstanceOfExpression(
    node: Node,
): node is InstanceofExpression {
    return (
        isBinaryExpression(node) &&
...
export function formatStringFromArgs(
    ...
    return text.replace(
        /{(\d+)}/g,
        (_match, index: string) => "" + diagnosticArgumentToText(Debug.checkDefined(args[+index])),
    ); // FIXME: needs to use the to text method if not a string
}
...
export function diagnosticArgumentToText(arg: DiagnosticArgument) {
    return typeof arg === "string" || typeof arg === "number" ? arg : arg.text;
}
...
/** @external */
export function createDiagnosticArgument<T extends string>(value: T): StructuredDiagnosticArgument;
export function createDiagnosticArgument<T extends Node>(value: T, toArgumentText: (t: T) => string, type: "Node"): StructuredDiagnosticArgument;
export function createDiagnosticArgument<T extends Symbol>(value: T, toArgumentText: (t: T) => string, type: "Symbol"): StructuredDiagnosticArgument;
export function createDiagnosticArgument<T extends Signature>(value: T, toArgumentText: (t: T) => string, type: "Signature"): StructuredDiagnosticArgument;
export function createDiagnosticArgument<T extends Type>(value: T, toArgumentText: (t: T) => string, type: "Type"): StructuredDiagnosticArgument;
export function createDiagnosticArgument<T extends TypePredicate>(value: T, toArgumentText: (t: T) => string, type: "TypePredicate"): StructuredDiagnosticArgument;
export function createDiagnosticArgument<T extends Node | Symbol | Signature | Type | TypePredicate | string>(
    value: T,
    toArgumentText?: (t: T) => string,
    type: "Node" | "Symbol" | "Signature" | "Type" | "TypePredicate" | "string" = "string",
): StructuredDiagnosticArgument {
    if (typeof value !== "string") {
        ...
        return { cacheId: idx, type, text: toArgumentText!(value) };
    }
    else {
        return { cacheId: undefined, type: "string", text: value };
    }
}
/** @internal */
...
export function createCompilerDiagnostic(
    message: DiagnosticMessage,
    ...rawArgs: (DiagnosticArgument | string)[]
): Diagnostic {
    let text = getLocaleSpecificMessage(message);

    const args = rawArgs.map(arg => typeof arg === "string" ? createDiagnosticArgument(arg) : arg);
    if (some(args)) {
        text = formatStringFromArgs(text, args);
    }
...
export function shouldPreserveConstEnums(
    ...
    return !!(
        compilerOptions.preserveConstEnums || getIsolatedModules(compilerOptions)
    );
}
...
export function getCompilerOptionValue(
    ...
    return option.strictFlag
        ...
        : option.allowJsFlag
        ? getAllowJSCompilerOption(options)
        : options[option.name];
}
        ...
        if (commonResolved && commonOriginal) {
            cache.setSymlinkedDirectory(commonOriginal, {
                ...
                realPath: ensureTrailingDirectorySeparator(
                    toPath(commonResolved, cwd, getCanonicalFileName),
                ),
            });
        }
    }
        ...
        ...mapDefined(extraFileExtensions, x =>
            x.scriptKind === ScriptKind.Deferred ||
                (needJsExtensions &&
                    ...
                    !flatBuiltins.includes(x.extension as Extension))
                ? [x.extension]
                : undefined),
    ];
...
export function createPropertyNameNodeForIdentifierOrLiteral(
    ...
    return !isMethodNamedNew && isIdentifierText(name, target)
    ...
    while (partEnd >= 0) {
        ...
        switch (state) {
            case States.BeforeNodeModules:
                if (
                    fullPath.indexOf(nodeModulesPathPart, partStart) === partStart
                ) {
 ...
 * @internal
 */
export function getPropertyNameFromType(
    ...
    if (type.flags & (TypeFlags.StringLiteral | TypeFlags.NumberLiteral)) {
        return escapeLeadingUnderscores(
            "" + (type as StringLiteralType | NumberLiteralType).value,
        );
    }
    return Debug.fail();
}
...
/** @internal */
export function hasResolutionModeOverride(
    node: ImportTypeNode | ImportDeclaration | ExportDeclaration | undefined,
) {
    if (node === undefined) {
        return false;
    }
